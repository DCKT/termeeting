...
describe("CliService", () => {
  ...
  const mockAccountStore = (accounts: any[] = [], defaultNickname?: string) =>
    Layer.succeed(AccountStore, {
      ...
      remove: () => Effect.void,
      getDefault: () =>
        Effect.succeed(defaultNickname ? Option.some(defaultNickname) : Option.none()),
    ...
    Effect.gen(function* () {
      ...
      Effect.provide(
        make({ timeZone: "UTC" }).pipe(
          ...
          Layer.provideMerge(mockAccountStore([{ nickname: "default", email: "test@example.com" }], "default")),
  ...
  it.effect("errors on unknown --account", () =>
    Effect.gen(function* () {
      const cli = yield* CliService
      const error = yield* cli.run(["--account", "nonexistent"]).pipe(Effect.flip)
      expect(error).toBeInstanceOf(CliError)
  ...
  describe("account add", () => {
          ...
          make({ timeZone: "UTC" }).pipe(
            ...
            Layer.provideMerge(
              Layer.succeed(ConfigStore, {
                read: () =>
                  Effect.succeed(Option.some({ clientId: "id", clientSecret: "secret" })),
                write: () => Effect.void,
              })
            ),
            Layer.provideMerge(mockTokenStore),
    ...
    it.effect("errors without config", () =>
      Effect.gen(function* () {
        const cli = yield* CliService
        const error = yield* cli.run(["account", "add", "work"]).pipe(Effect.flip)
        expect(error).toBeInstanceOf(CliError)
  ...
  describe("account set-default", () => {
    ...
    it.effect("errors without nickname", () =>
      Effect.gen(function* () {
        const cli = yield* CliService
        const error = yield* cli.run(["account", "set-default"]).pipe(Effect.flip)
        expect(error).toBeInstanceOf(CliError)
        expect(error.message).toContain("Usage")
      }).pipe(
        Effect.provide(
